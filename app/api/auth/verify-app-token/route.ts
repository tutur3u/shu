...
function normalizeApiBaseUrl() {
	return getShuApiBaseUrl().replace(/\/+$/, "");
}
...
function toShuSession(payload: AppTokenExchangeResponse): ShuAdminSession {
	...
	return {
		...
		app: {
			name: payload.app?.name ?? getShuAppId(),
		},
...
export async function POST(request: NextRequest) {
	try {
		...
		const session = toShuSession(await exchangeCrossAppToken(token));
		...
		setShuSessionCookie(response, session);
		return response;
	} catch (error) {
		console.error("[shu:auth] app token exchange failed", error);
