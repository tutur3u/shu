...
import type { ShuAdminSession } from "./shu-session";
...
function createSession(): ShuAdminSession {
	return {
...
function readSessionCookieValue(response: NextResponse) {
	...
	expect(setCookie).toContain("shu_admin_session=");
	return setCookie?.split(";")[0]?.slice("shu_admin_session=".length) ?? "";
}

describe("shu session validation", () => {
	beforeEach(() => {
		process.env.TUTURUUU_API_BASE_URL = "https://platform.example.com/api/v1";
		process.env.TUTURUUU_SHU_WORKSPACE_ID = "ws-linked";
		process.env.SHU_SESSION_SECRET = "session-secret";
	...
	afterEach(() => {
		...
		delete process.env.TUTURUUU_SHU_WORKSPACE_ID;
		delete process.env.SHU_SESSION_SECRET;
		sessionCookieValue = null;
	});

	for (const status of [401, 403, 404] as const) {
		test(`rejects stored sessions when platform revalidation returns ${status}`, async () => {
			const { getShuSessionFromCookies, setShuSessionCookie } = await import(
				"./shu-session"
			);
			const response = NextResponse.json({});
			setShuSessionCookie(response, createSession());
			...
			await expect(getShuSessionFromCookies()).resolves.toBeNull();
